...
export default function CTAForm({ defaultServiceType = "" }: CTAFormProps) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await submitLead(data, "https://formspree.io/f/mqenakjz");
      if (res.ok) {
        setSubmitted(true);
      } else {
        setError(true);
    ...
    return (
      <div className="text-center py-12">
        ...
        <p className="text-white/90">
          Estudiamos tu petición y te contactaremos antes de 24 horas con una valoración inicial.
        </p>
      </div>
    );
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      <input type="hidden" name="_subject" value="Nuevo presupuesto desde arcreformastarragona.es (CTA)" />
      ...
      <div>
        <label className="block text-white font-bold text-xs uppercase tracking-wider mb-2">Tipo de obra</label>
        <select name="serviceType" defaultValue={defaultServiceType} required className="w-full px-4 py-3 rounded-xl bg-white/20 text-white border-none focus:outline-none focus:ring-2 focus:ring-white/40">
          ...
          <option value="Piso del Eixample / centro" className="text-dark">Piso del Eixample / centro</option>
          <option value="Edificio Part Alta / catalogado" className="text-dark">Edificio Part Alta / catalogado</option>
      ...
      <div>
        <label className="block text-white font-bold text-xs uppercase tracking-wider mb-2">Detalle del proyecto</label>
        <textarea name="message" placeholder="Metros útiles, año del edificio, estado de las instalaciones, qué te gustaría priorizar..." rows={4} className="w-full px-4 py-3 rounded-xl bg-white/20 text-white placeholder:text-white/50 border-none focus:outline-none focus:ring-2 focus:ring-white/40 resize-vertical" />
      </div>
      ...
      <div className="flex items-start gap-2">
        ...
        <label htmlFor="lopd" className="text-white/70 text-xs leading-relaxed">
          He leído y acepto la <a href="/politica-privacidad" className="underline text-white/90">política de privacidad</a> para que ARC Reformas Tarragona estudie mi petición y me envíe una valoración.
        </label>
      </div>
      ...
      >
        ...
        {loading ? "Enviando..." : "Pedir valoración sin coste"}
      </button>
