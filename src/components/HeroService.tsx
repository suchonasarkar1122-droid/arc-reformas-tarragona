    ...
    <section className={`relative ${height} w-full overflow-hidden`}>
      ...
      <div className="relative z-10 h-full flex flex-col items-center justify-center text-center px-5">
        <div className="max-w-[900px] flex flex-col items-center gap-5">
          ...
          {tagline && !badge && (
            <span className="inline-block font-display font-bold text-xs uppercase tracking-[0.3em] text-accent">{tagline}</span>
          )}

          {h1Keyword ? (
            <>
              <h1 className="font-display text-accent text-sm md:text-base uppercase tracking-[0.2em] font-semibold">{h1Keyword}</h1>
              <h2 className="font-display font-black text-[36px] md:text-[56px] lg:text-[72px] text-white uppercase leading-[0.95]">
                {title}
              </h2>
            </>
          ) : (
            <h1 className="font-display font-black text-[36px] md:text-[56px] lg:text-[72px] text-white uppercase leading-[0.95]">
              {title}
            </h1>
          )}
            ...
            <div className="flex flex-wrap items-center justify-center gap-x-6 gap-y-2 text-white/70 text-sm">
              {trustItems.map((item, i) => (
                <span key={i} className="flex items-center gap-1.5">
                  {i === 0 && (
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="#E8612A">
